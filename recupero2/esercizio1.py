...
if __name__ == "__main__":
    ...
    s1 = input("Inserisci la prima sequenza di DNA (s1): ").upper().strip()
    s2 = input("Inserisci la seconda sequenza di DNA (s2): ").upper().strip()

    if not isDNA(s1) or not isDNA(s2):
        print("Errore: Una o entrambe le sequenze non sono valide. Devono contenere solo A, C, G o T.")
