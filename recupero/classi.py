...
class ContactManager:
    ...
    def create_contact(self, name, phone_numbers):
        if name in self.contacts:
            return "Errore: il contatto esiste già."
        self.contacts[name] = phone_numbers
        return {name: phone_numbers}

    def add_phone_number(self, contact_name, phone_number):
    ...
    def remove_phone_number(self, contact_name, phone_number):
        if contact_name not in self.contacts:
            return "Errore: il contatto non esiste."
    ...
    def update_phone_number(self, contact_name, old_phone_number, new_phone_number):
        if contact_name not in self.contacts:
            return "Errore: il contatto non esiste."
        if old_phone_number not in self.contacts[contact_name]:
            return "Errore: il numero di telefono non è presente."
        new_list = []
        for num in self.contacts[contact_name]:
            if num == old_phone_number:
    ...
    def search_contact_by_phone_number(self, phone_number):
        search = [name for name, numbers in self.contacts.items() if phone_number in numbers]
        if not search:
