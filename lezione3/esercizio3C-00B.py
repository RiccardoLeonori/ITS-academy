'''Esercizio 3C-00B. 
Scrivere un programma in Python che chieda all'utente di inserire il proprio nome e 
il proprio genere (specificato con "m" per maschio o "f" per femmina) e utilizzi lo statement match 
...
genere = str(input("Scrivi il tuo genere (inserisci \"m\" per maschio o \"f\" per femmina): "))
match genere:
    case "m":
        print(f"Nome: {nome}\nGender: Maschio")
    case "f":
        print(f"Nome: {nome}\nGender: Femmina")
    case _:
        print(f"Mi dispiace {nome}, non è possibile generare un documento d'identità!")